...
void main() {
    ...
    vec4 shadowMapPosition = lightData.light_ViewProjectionMatrix * texture(gBufferPosition, screenSpaceCoordinates);
    ...
    // Transform the fragment's light NDC Space position into the shadowMap's Image Space.
    ...
    if (shadowMapPosition.z > shadowDepth) renderColor *= vec4(0.1, 0.1, 0.1, 1.0);
}