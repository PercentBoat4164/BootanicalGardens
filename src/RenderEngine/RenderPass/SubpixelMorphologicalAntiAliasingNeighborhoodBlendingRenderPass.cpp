#include "SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass.hpp"
...
SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass::SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass(RenderGraph& graph) : RenderPass(graph) {
  material = graph.device->getJSONNonObjectMaterial(7);  // Subpixel Morphological Anti-Aliasing | Neighborhood Blending
  ...
  graph.setImageParameters(RenderGraph::getAttachmentId("SMAAResults"), parameters);
  graph.addImageUsageParameters(RenderGraph::RenderColor, VK_IMAGE_USAGE_TRANSFER_DST_BIT);
}
...
void SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass::bake(const std::vector<VkAttachmentDescription>& attachmentDescriptions, const std::vector<const Image*>& images) {
  ...
  uniformBuffer = std::make_unique<UniformBuffer<PassData>>(graph.device, "SMAA Neighborhood Blending Render Pass | Uniform Buffer");
}

void SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass::writeDescriptorSets(std::deque<std::tuple<void*, std::function<void(void*)>>>& miscMemoryPool, std::vector<VkWriteDescriptorSet>& writes) {
  const uint32_t offset = writes.size();
  /** todo: Make this more than just a proof of concept. **/
  // for (Material* material : pipelines | std::ranges::views::keys) {
  //   for (auto& [binding, descriptor] : material->perSetBindings[1]) {
  //     /** todo: Must deduplicate bindings so that we only write each binding once. P.S. Bindings must agree. **/
  //     switch (descriptor.id) {
  //       case Tools::hash("resolution"):
  ...
  //         });
  //         break;
  //       default: GraphicsInstance::showError("failed to bind!");
  //     }
  //   }
  // }
  writes.resize(offset + descriptorSets.size(), {
...
void SubpixelMorphologicalAntiAliasingNeighborhoodBlendingRenderPass::execute(CommandBuffer& commandBuffer) {
