...
RenderGraph::PerFrameData::PerFrameData(const GraphicsDevice& device) : device(device) {
  /****************************************
...
const RenderGraph::PerFrameData& RenderGraph::getPerFrameData() const {
  return frames[frameNumber % FRAME_OVERLAP];
}

RenderGraph::OneTimeSubmit::CommandBuffer::CommandBuffer(const GraphicsDevice& device, VkCommandPool commandPool) : device(device) {
  const VkCommandBufferAllocateInfo commandBufferAllocateInfo{
      .sType              = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
      .pNext              = nullptr,
      .commandPool        = commandPool,
...
RenderGraph::OneTimeSubmit::CommandBuffer RenderGraph::OneTimeSubmit::CommandBuffer::begin() {
...
void RenderGraph::OneTimeSubmit::CommandBuffer::finish() {
...
RenderGraph::OneTimeSubmit::CommandBuffer::operator VkCommandBuffer() const {
  return commandBuffer;
}

RenderGraph::OneTimeSubmit::CommandBuffer::operator bool() const {
  return inUse;
}

RenderGraph::OneTimeSubmit::CommandBuffer RenderGraph::OneTimeSubmit::get() {
  const VkCommandPoolCreateInfo commandPoolCreateInfo{
      .sType            = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,
      .pNext            = nullptr,
...
RenderGraph::OneTimeSubmit::OneTimeSubmit(const GraphicsDevice& device) : device(device) {}

RenderGraph::OneTimeSubmit::~OneTimeSubmit() {
  vkDestroyCommandPool(device.device, commandPool, nullptr);
  commandPool = VK_NULL_HANDLE;
}

RenderGraph::RenderGraph(const GraphicsDevice& device) : device(device), oneTimeSubmit(device), drawImage(device, "drawImage"), descriptorAllocator(device) {
...
void RenderGraph::setup(const std::vector<Image>& swapchainImages) {
...
VkSemaphore RenderGraph::waitForFrameData() const {
  const PerFrameData& frameData = getPerFrameData();
...
std::vector<VkSemaphore> RenderGraph::render(const uint32_t swapchainIndex, const Image& swapchainImage) const {
  const PerFrameData& frameData = getPerFrameData();
