#include "SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass.hpp"
...
SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass::SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass(RenderGraph& graph) : RenderPass(graph) {
  material = graph.device->getJSONNonObjectMaterial(5);  // Subpixel Morphological Anti-Aliasing | Luma Edge Detection
  ...
  graph.setImageParameters(RenderGraph::getAttachmentId("SMAAEdges"), parameters);
}
...
void SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass::bake(const std::vector<VkAttachmentDescription>& attachmentDescriptions, const std::vector<const Image*>& images) {
  ...
  uniformBuffer = std::make_unique<UniformBuffer<PassData>>(graph.device, "SMAA Edge Detection Render Pass | Uniform Buffer");
}

void SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass::writeDescriptorSets(std::deque<std::tuple<void*, std::function<void(void*)>>>& miscMemoryPool, std::vector<VkWriteDescriptorSet>& writes) {
...
void SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass::execute(CommandBuffer& commandBuffer) {
