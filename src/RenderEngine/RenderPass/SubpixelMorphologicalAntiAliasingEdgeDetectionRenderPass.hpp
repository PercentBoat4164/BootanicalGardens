...
class SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass : public RenderPass {
  struct PassData { glm::vec4 resolution; };
  std::unique_ptr<UniformBuffer<PassData>> uniformBuffer;

public:
  explicit SubpixelMorphologicalAntiAliasingEdgeDetectionRenderPass(RenderGraph& graph);
